...
RUN cp -r /build/libdavs2/build/linux /build/libdavs2/build/darwin \
    ...
    --host=aarch64-apple-darwin24.1 \
    --cross-prefix=aarch64-apple-darwin24.1- \
    && make -j$(nproc) && make install \
    && rm -rf /build/libdavs2

# librav1e
RUN cd /build/librav1e \
    && cargo cinstall -v --target=aarch64-apple-darwin --prefix=${PREFIX} --library-type=staticlib --crt-static --release \
...
CMD ["cp", "/build/ffmpeg-7.1-darwin-arm64.tar.gz", "/output"]